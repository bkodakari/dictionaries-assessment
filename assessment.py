...
def get_melon_price(melon_name):
    ...
    for melon in melon_prices:
        if melon_name in melon_prices.keys():
...
def word_length_sorted(words):
    ...
    for word in words:
        ...
        if key not in words:
            ...
            words_by_length[key] = words_by_length[key].append(word)
            # Cannot figure out how to add the value to the list, let alone aphabetize it
...
def kids_game(names):
    ...
    for name in names:
        ...
        if key not in names_by_first_letter:
            names_by_first_letter[key] = [name]
        else:
            names_by_first_letter[key].append(name)

    fun_list = [names[0]]

    key = fun_list[0][len(fun_list[0]) - 1]

    while True:
        try:
            next_name = random.choice(names_by_first_letter[key])
            if next_name not in fun_list:
                fun_list.append(next_name)
